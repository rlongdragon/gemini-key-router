...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "gemini-key-router" is now active!'
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("gemini-key-router.openPanel1", () => {
      if (panel1) {
        ...
        panel1.webview.html = getWebviewContent(
          context,
          panel1.webview,
          "panel1"
        );
...
function getWebviewContent(
  ...
  if (styleMatch && styleMatch[1]) {
    ...
    const styleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(context.extensionUri, "webview-ui", "dist", stylePath)
    );
    finalHtmlContent = finalHtmlContent.replace(
      styleMatch[0],
  ...
  finalHtmlContent = finalHtmlContent
    .replace(
      "</head>",
      `<script nonce="${nonce}">window.panelType = '${panelType}';</script></head>`
    )
    .replace(
      /<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src {{cspSource}}; script-src 'nonce-{{nonce}}';">/g,
